...
parser.add_argument("--mode", default='MS-SSIM', choices=['PSNR', 'MS-SSIM'])
...
parser.add_argument("--l", type=int, default=32, choices=[8, 16, 32, 64, 256, 512, 1024, 2048])
...
#path_rand = folder[np.random.randint(len(folder))] + '/'
#path1 = folder[30] + '/'

path = 'dataset_one_folder_split'
folder = os.path.join(path, 'folder2')
imlist = [os.path.join(folder, x) for x in os.listdir(folder)]

path_com = folder + '_com_' + args.mode  + '_' + str(args.l) + '/'
path_bin = folder + '_bin_' + args.mode  + '_' + str(args.l) + '/'

path_com = 'results/PSNR_2048_one_folder_minmax/' + '_com_' + args.mode + '_' + str(args.l) + '/'
path_bin = 'results/PSNR_2048_one_folder_minmax/' + '_bin_' + args.mode + '_' + str(args.l) + '/'

path_com = 'results/MSSIM_32_one_folder_minmax/' + '_com_' + args.mode + '_' + str(args.l) + '/'
path_bin = 'results/MSSIM32_one_folder_minmax/' + '_bin_' + args.mode + '_' + str(args.l) + '/'

path_com = 'results/GAN_32_N_256_PSNR_1024/' + '_com_' + args.mode + '_' + str(args.l) + '/'
path_bin = 'results/GAN_32_N_256_PSNR_1024/' + '_bin_' + args.mode + '_' + str(args.l) + '/'

os.makedirs(path_com, exist_ok=True)
os.makedirs(path_bin, exist_ok=True)
F1 = np.expand_dims(dcmread(imlist[0]).pixel_array, axis = 2)
#F1 = misc.imread(path + 'f001.png')
Height = 736 #np.size(F1, 0)
Width = 64 #np.size(F1, 1)
...
print('=================================')
print(Y0_com.shape, "SHAPE OF FRAME")
print(Y1_raw.shape)
...
print(Res_hat.shape, 'AAAAAAAAAAAAAAAAA')
...
model_path = './OpenDVC_' + args.mode + '_' + str(args.l)+ 'one_folder' + '/model.ckpt-250'

#saver = tf.train.import_meta_graph(model_path) #Data loss: not an sstable (bad magic number):
...
for gop_number in range(1):

    #f = gop_number * args.GOP
    frames = args.GOP
    f = frames*gop_number #start frame
    ...
    bits = os.path.getsize(imlist[f])
    #bits = bits * 8

    #F0_com = misc.imread(path_com + 'f' + str(f + 1).zfill(3) + '.png')
    #F0_raw = misc.imread(path + 'f' + str(f + 1).zfill(3) + '.png')
    F0_com = np.expand_dims(dcmread(imlist[f]).pixel_array, axis = 2)
    F0_raw = np.expand_dims(dcmread(imlist[f]).pixel_array, axis = 2)
    print(np.unique(F0_com), 'NNNNNNNNNNNNNNNN')
    plt.imsave(os.path.join(path_com, str(f)+'.png'), np.squeeze(F0_com), cmap='gray')

    F0_com = np.expand_dims(F0_com, axis=0)
    F0_raw = np.expand_dims(F0_raw, axis=0)

    if args.metric == 'PSNR':
        mse = np.mean(np.power(np.subtract(F0_com / 65534.0, F0_raw / 65534.0), 2.0))
        quality_frame[f] = 10 * np.log10(1.0 / mse)
    elif args.metric == 'MS-SSIM':
        quality_frame[f] = MultiScaleSSIM(F0_com, F0_raw, max_val=65534)
    ...
    for p_frame in range(frames-1):

        F1_raw = np.expand_dims(dcmread(imlist[p_frame+f+1]).pixel_array, axis = 2) #misc.imread(path + 'f' + str(f + 1).zfill(3) + '.png')
        F1_raw = np.expand_dims(F1_raw, axis=0)

        F0_com, string_MV, string_Res, quality_com \
            = sess.run([Y1_com, string_mv, string_res, quality],
                       feed_dict={Y0_com: F0_com / 65534.0, Y1_raw: F1_raw / 65534.0})

        F0_com = F0_com * 65534.0

        with open(path_bin + str(p_frame+f+1).zfill(3) + '.bin', "wb") as ff:
        ...
        print(F0_com.shape, 'FO COM', F0_com[0].shape)
        print(F0_com)
        #print(os.path.join(path_com, str(f[:-4])+'.png'))
        #imageio.imwrite(os.path.join(path_com, str(f[:-4])+'.png'),np.squeeze(F0_com[0]))
        plt.imsave(os.path.join(path_com, str(f+p_frame)+'.png'), np.squeeze(F0_com[0]), cmap='gray')

        bits = (2 + len(string_MV) + len(string_Res)) * 8

        quality_frame[p_frame+f] = quality_com
        bits_frame[p_frame+f] = bits

        print('Frame', p_frame+f, args.metric + ' =', quality_frame[p_frame+f], 'bpp =', bits_frame[p_frame+f] / Height / Width)
