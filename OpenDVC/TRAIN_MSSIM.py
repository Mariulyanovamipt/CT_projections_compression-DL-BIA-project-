...
exper = Experiment(api_key = "SCNv0oqcoLPJnXIqei3EvhuKR",project_name = "DVC_1_folder_MSSIM_32_minmax", workspace = 'mariulyanovamipt')
...
Y1_MC = MC_network.MC(MC_input, filters_final = Channel)

#print('aaaaaaaaaaaaaaaaaaaaaaaaa', Y1_warp.shape, Y0_com.shape, MC_input.shape, Y1_MC.shape, flow_hat.shape)

# Encode residual
Res = Y1_raw - Y1_MC

res_latent = CNN_img.Res_analysis(Res, num_filters=args.N, M=args.M) #, channels = Channel)
...
print(Res_hat.shape, 'AAAAAAAAAA')
print(Y1_MC.shape)
...
print('AAAAAAAAAAAAAAAAAa', Y1_com.shape)
print(Y1_raw.shape)
print(type(Y1_raw[0][0]), 'dlffjldldjjl')
...
save_path = './OpenDVC_MS-SSIM_' + str(l) + 'one_folder'
...
saver_psnr.restore(sess, save_path='OpenDVC_PSNR_1024one_folder_minmax_norm/model.ckpt-3000')
