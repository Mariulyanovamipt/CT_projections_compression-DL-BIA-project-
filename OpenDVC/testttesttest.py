...
path_com = 'dataset_video_my/' + '_com_' +'PSNR'+ '_' + str(1024) + '/'
path_bin = 'dataset_video_my/' + '_bin_' + 'PSNR'+ '_' + str(1024) + '/'


F1 = np.expand_dims(dcmread(imlist[0]).pixel_array, axis = 2)
F1 = dcmread(imlist[0]).pixel_array

plt.imsave(os.path.join(path_com, 'AAAAAAAAAAAAA'+'.png'), F1, cmap='gray')

arr = dcmread(imlist[0]).pixel_array # or np.ones etc.


array_buffer = dcmread(imlist[0]).PixelData
...
exper = Experiment(api_key = "SCNv0oqcoLPJnXIqei3EvhuKR",
                   project_name = "DVC_1_folder_PSNR_2048_normalized_minmax", #the same forlder as for 1024
...
parser.add_argument("--l", type=int, default=2048, choices=[256, 512, 1024, 2048])
parser.add_argument("--N", type=int, default=128, choices=[128, 256])
parser.add_argument("--M", type=int, default=128, choices=[128, 256])
...
#exper.log_metric('mse', total_mse)
#exper.log_metric('psnr', psnr)
#exper.log_metric('bits_total', train_bpp_MV + train_bpp_Res)
...
saver.restore(sess, save_path='OpenDVC_PSNR_2048one_folder/model.ckpt-2000')
