...
import imageio # use instead of misc
import CNN_img
import motion
import MC_network
import os
...
imlist_png = [os.path.join(path,im) for im in os.listdir(path) if im[-4:]=='.png']
#F1 = dcmread(imlist[0]).pixel_array
F1 = imageio.imread(imlist_png[0])
print(imlist_png[0])
print(F1.shape)
...
for gop_number in range(np.int(np.ceil(args.frame/args.GOP))):
    ...
    if args.mode == 'PSNR':
        ...
        os.system('bpgenc -f 444 -m 9 ' + filename+' -o ' + path_bin + str(f + 1).zfill(3) + '.bin -q ' + str(I_QP))
        os.system('bpgdec ' + path_bin + str(f + 1).zfill(3) + '.bin -o ' + path_com + 'f' + str(f + 1).zfill(3) + '.png')
